...
class TransitionSignal(object):
    ...
    def __enter__(self):
        if self.status:
            ...
            LOGGER.debug(
                "The signal that is fired right before the transition ( %s ) happened for %s"
    ...
    def __exit__(self, type, value, traceback):
        if self.status:
            ...
            LOGGER.debug(
                "The signal that is fired right after the transition ( %s) happened for %s"
                % (self.transition_approval.transition, self.workflow_object)
            )
...
class ApproveSignal(object):
    ...
    def __enter__(self):
        ...
        LOGGER.debug(
            "The signal that is fired right before a transition approval is approved for %s due to transition %s -> %s"
            % (
                self.workflow_object,
                self.transition_approval.transition.source_state.label,
                self.transition_approval.transition.destination_state.label,
            )
        )
    ...
    def __exit__(self, type, value, traceback):
        for hook in OnApprovedHook.objects.filter(
            ...
            hook.execute(self._get_context(AFTER))
        LOGGER.debug(
            "The signal that is fired right after a transition approval is approved for %s due to transition %s -> %s"
            % (
                self.workflow_object,
                self.transition_approval.transition.source_state.label,
                self.transition_approval.transition.destination_state.label,
    ...
    def __enter__(self):
        if self.status:
            for hook in OnCompleteHook.objects.filter(
                ...
                hook.execute(self._get_context(BEFORE))
            LOGGER.debug(
                "The signal that is fired right before the workflow of %s is complete"
                % self.workflow_object
            )
    ...
    def __exit__(self, type, value, traceback):
        if self.status:
            for hook in OnCompleteHook.objects.filter(
                ...
                hook.execute(self._get_context(AFTER))
            LOGGER.debug(
                "The signal that is fired right after the workflow of %s is complete"
                % self.workflow_object
            )
